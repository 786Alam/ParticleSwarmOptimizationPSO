...
def calculate_individual_interaction_score(vehicle_id, all_vehicle_ids):
    ...
    if follower_id and follower_id in traci.vehicle.getIDList():
        ...
        if vehicle_type == 'auto' and follower_type == 'car':
            interaction_score += 1
            print(f"Autonomous Vehicle {vehicle_id} is too close to the Following Normal Vehicle ({follower_id}),\n distance: {distance}\n")
    ...
    if follower_id and follower_id in traci.vehicle.getIDList():
        ...
        if vehicle_type == 'auto' and follower_type == 'car':
            if distance < MIN_FOLLOW_DISTANCE:
                interaction_score += 1
                print(f"Autonomous Vehicle {vehicle_id} is too close to the Following Normal Vehicle ({follower_id}),\n distance: {distance}\n")
                traci.vehicle.slowDown(vehicle_id, 5.0, 1)  # Slow down to increase distance
                new_distance = ((x1 - x2) ** 2 + (y1 - y2) ** 2) ** 0.5
                print(f"Autonomous Vehicle {vehicle_id} increased distance from FOLLOWING NORMAL VEHICLE, new distance: {new_distance}\n")
        ...
        if leader_id in traci.vehicle.getIDList():
            if leader_distance < MIN_LEADER_DISTANCE:
                ...
                if vehicle_type == 'auto' and leader_type == 'auto':
                    ...
                    print(f"Autonomous Vehicle {vehicle_id} is too close to the Leading Autonomous Vehicle ({leader_id}),\n distance: {leader_distance}\n")
                    traci.vehicle.slowDown(vehicle_id, 5.0, 1)  # Slow down to increase distance
                    new_leader_distance = traci.vehicle.getDistance(vehicle_id, leader_id)
                    ...
                    if traci.vehicle.getNextTLS(vehicle_id) == [] and traci.vehicle.getLaneChangeState(vehicle_id) == 0:
                        new_lane_index = traci.vehicle.getLaneIndex(vehicle_id) + 1
                        print(f"Autonomous Vehicle {vehicle_id} is blocked. Changing lane to {new_lane_index}.")
                        traci.vehicle.changeLane(vehicle_id, new_lane_index, 10.0)  # Change to the next lane
                        print(f"{vehicle_id} changed lane to {new_lane_index} to increase distance from LEADING AUTONOMOUS VEHICLE\n")
            ...
            if speed_difference > MAX_SPEED_DIFF:
                ...
                if vehicle_type == 'auto' and leader_type == 'auto':
                    ...
                    print(f"Autonomous Vehicle {vehicle_id} increased distance from SIGNIFICANTLY FASTER LEADING AUTONOMOUS VEHICLE, new speed difference: {new_speed_difference}\n")
            ...
            if vehicle_type == 'auto' and neighbor_type == 'car':
                if distance < MIN_LANE_CHANGE_DISTANCE:
                    ...
                    if traci.vehicle.getNextTLS(vehicle_id) == [] and traci.vehicle.getLaneChangeState(vehicle_id) == 0:
                        ...
                        print(f"Autonomous Vehicle {vehicle_id} is blocked. Changing lane to {new_lane_index}.")
                        traci.vehicle.changeLane(vehicle_id, new_lane_index, 10.0)  # Change to the next lane
                        print(f"{vehicle_id} changed lane to {new_lane_index} to increase distance from NEIGHBOR NORMAL VEHICLE IN A DIFFERENT LANE\n")
        ...
        if blocker_id != vehicle_id and blocker_id in traci.vehicle.getIDList():
            ...
            if vehicle_type == 'auto' and blocker_type == 'car':
                ...
                if traci.vehicle.getNextTLS(vehicle_id) == [] and traci.vehicle.getLaneChangeState(vehicle_id) == 0:
                    ...
                    print(f"Autonomous Vehicle {vehicle_id} is blocked. Changing lane to {new_lane_index}.")
                    traci.vehicle.changeLane(vehicle_id, new_lane_index, 10.0)  # Change to the next lane
                    print(f"{vehicle_id} changed lane to {new_lane_index} to increase distance from BLOCKED NORMAL VEHICLE\n")
...
def get_interaction_info(vehicle_id):
    ...
    if vehicle_type_with_prefix.startswith('auto'):
        ...
        if follower_id and follower_id in traci.vehicle.getIDList():
            ...
            traci.vehicle.slowDown(vehicle_id, 5.0, 1)  # Slow down to increase distance
            new_distance = ((x1 - x2) ** 2 + (y1 - y2) ** 2) ** 0.5
            info += f"Autonomous Vehicle {vehicle_id} increased distance from FOLLOWING NORMAL VEHICLE, new distance: {new_distance}\n"
        ...
        if leader_id_info is not None:
            ...
            if leader_id in traci.vehicle.getIDList():
                if leader_distance < MIN_LEADER_DISTANCE:
                    ...
                    traci.vehicle.slowDown(vehicle_id, 5.0, 1)  # Slow down to increase distance
                    new_leader_distance = traci.vehicle.getDistance(vehicle_id, leader_id)
                    info += f"Autonomous Vehicle {vehicle_id} increased distance from LEADING AUTONOMOUS VEHICLE, new distance: {new_leader_distance}\n"

                    # Check if the autonomous vehicle is blocked and change lane
                    if traci.vehicle.getNextTLS(vehicle_id) == [] and traci.vehicle.getLaneChangeState(vehicle_id) == 0:
                        new_lane_index = traci.vehicle.getLaneIndex(vehicle_id) + 1
                        info += f"Autonomous Vehicle {vehicle_id} is blocked. Changing lane to {new_lane_index}."
                        traci.vehicle.changeLane(vehicle_id, new_lane_index, 10.0)  # Change to the next lane
                        info += f"{vehicle_id} changed lane to {new_lane_index} to increase distance from LEADING AUTONOMOUS VEHICLE\n"
                ...
                if speed_difference > MAX_SPEED_DIFF:
                    ...
                    traci.vehicle.slowDown(vehicle_id, 5.0, 1)  # Slow down to increase distance
                    new_speed_difference = vehicle_speed - traci.vehicle.getSpeed(leader_id)
                    info += f"Autonomous Vehicle {vehicle_id} increased distance from SIGNIFICANTLY FASTER LEADING AUTONOMOUS VEHICLE, new speed difference: {new_speed_difference}\n"
                ...
                for neighbor_id, _ in neighbors:
                    if neighbor_id in traci.vehicle.getIDList():
                        ...
                        traci.vehicle.slowDown(vehicle_id, 5.0, 1)  # Slow down to increase distance
                        new_distance = ((x1 - x2) ** 2 + (y1 - y2) ** 2) ** 0.5
                        info += f"Autonomous Vehicle {vehicle_id} increased distance from NEIGHBOR NORMAL VEHICLE IN A DIFFERENT LANE, new distance: {new_distance}\n"
                        ...
                        if traci.vehicle.getNextTLS(vehicle_id) == [] and traci.vehicle.getLaneChangeState(vehicle_id) == 0:
                            ...
                            info += f"Autonomous Vehicle {vehicle_id} is blocked. Changing lane to {new_lane_index}."
                            traci.vehicle.changeLane(vehicle_id, new_lane_index, 10.0)  # Change to the next lane
                            info += f"{vehicle_id} changed lane to {new_lane_index} to increase distance from NEIGHBOR NORMAL VEHICLE IN A DIFFERENT LANE\n"
                ...
                for blocker_id, _ in blockers:
                    if blocker_id != vehicle_id and blocker_id in traci.vehicle.getIDList():
                        ...
                        traci.vehicle.slowDown(vehicle_id, 5.0, 1)  # Slow down to increase distance
                        new_distance = ((x1 - x2) ** 2 + (y1 - y2) ** 2) ** 0.5
                        info += f"Autonomous Vehicle {vehicle_id} increased distance from BLOCKED NORMAL VEHICLE, new distance: {new_distance}\n"
                        ...
                        if traci.vehicle.getNextTLS(vehicle_id) == [] and traci.vehicle.getLaneChangeState(vehicle_id) == 0:
                            ...
                            info += f"Autonomous Vehicle {vehicle_id} is blocked. Changing lane to {new_lane_index}."
                            traci.vehicle.changeLane(vehicle_id, new_lane_index, 10.0)  # Change to the next lane
                            info += f"{vehicle_id} changed lane to {new_lane_index} to increase distance from BLOCKED NORMAL VEHICLE\n"
