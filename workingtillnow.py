...
def get_fitness(vehicle_ids):
    ...
    travel_time = travel_times().get(reference_vehicle_id, 0)
...
def pso_algorithm():
    ...
    # Initialize global best positions and velocities for each vehicle
    global_best_positions = {vehicle_id: np.random.uniform(low=0, high=25, size=3).tolist() for vehicle_id in vehicle_ids}
    velocity_memory = {vehicle_id: np.zeros(3).tolist() for vehicle_id in vehicle_ids}

    # Initialize personal bests
    personal_best_positions = {vehicle_id: global_best_positions[vehicle_id][:] for vehicle_id in vehicle_ids}
    personal_best_fitness = {vehicle_id: float('-inf') for vehicle_id in vehicle_ids}

    for iteration in range(MAX_ITERATIONS):
        print(f"Iteration: {iteration + 1}/{MAX_ITERATIONS}")

        for vehicle_id in vehicle_ids:
            current_position = global_best_positions[vehicle_id]
            current_velocity = velocity_memory[vehicle_id]

            # Update velocity and position
            new_velocity = update_velocity(current_velocity, personal_best_positions[vehicle_id], global_best_positions[vehicle_id], current_velocity)
            new_position = [pos + vel for pos, vel in zip(current_position, new_velocity)]

            # Update the vehicle's attributes in the simulation
            update_vehicle_attributes(vehicle_id, new_position)

            # Evaluate the new position's fitness after the simulation step
            fitness = evaluate_particle(vehicle_id)
            print(fitness)

            # Update personal and global bests if the new position is better
            if fitness > personal_best_fitness[vehicle_id]:
                personal_best_fitness[vehicle_id] = fitness
                personal_best_positions[vehicle_id] = new_position

            if fitness > evaluate_particle(vehicle_id):  # Re-evaluating fitness for the global best comparison
                global_best_positions[vehicle_id] = new_position

            # Update velocity memory
            velocity_memory[vehicle_id] = new_velocity
...
def run_pso_experiment():
    ...
    for step in range(SIMULATION_STEPS):
        traci.simulationStep()
        display_entered_and_left_vehicles(step)
        ...
        # Run PSO algorithm
        global_best_positions = pso_algorithm()
...
def append_data_to_results(step, collision_follower_avoided, fitness_penalty_follow, collision_leader_avoided, fitness_penalty_lead, collision_neighbor_avoided, fitness_penalty_neighbor,
    ...
    print(f"Step {step} - Normal Vehicles Count: {normal_vehicles_count}, Autonomous Vehicles Count: {autonomous_vehicles_count}")
    ...
    # Append the step_data lists to the result_data list
    result_data.append(step_data)
