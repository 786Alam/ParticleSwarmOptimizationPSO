...
def calculate_individual_interaction_score(vehicle_id):
    ...
    MIN_LEADER_DISTANCE = 5
    MIN_LANE_CHANGE_DISTANCE = 5
    MAX_SPEED_DIFF = 5 
...
def pso_algorithm():
    ...
    for vehicle_id in vehicle_ids:
        ...
        velocities[vehicle_id] = abs(velocities[vehicle_id])
        ...
        if fitness < get_fitness(vehicle_id):  
            personal_bests[vehicle_id] = (x, y)
...
def append_data_to_results(iteration, collision_follower_avoided, fitness_penalty_follow, collision_leader_avoided, fitness_penalty_lead, collision_neighbor_avoided, fitness_penalty_neighbor,
                            change_lane_due_to_block, fitness_penalty_collision, interaction_details, result_data, distances, speeds, travel_times_data, positions, velocities, personal_bests, global_best_position, fitness_values, global_best_fitness):
